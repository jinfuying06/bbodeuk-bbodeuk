...
> **예외 (2026-09-17 시작, UI 업그레이드 이니셔티브)**: 이번 이니셔티브는 위 원칙의 예외로,
> 다음 항목에 한해 의도적인 개선을 허용한다 — 모션, 벡터 아이콘, 시각적 디테일, 컬러 토큰화.
> 컬러 토큰화에는 Figma 디자인 시스템(`Sky/BB` 팔레트, 민트/틸 계열)을 기준으로 한
> 기존 블루 계열(MD3) 팔레트의 전면 교체가 포함된다. Figma 쪽이 최신 확정본이며,
> 현재 코드의 팔레트는 구버전으로 확인되었다(2026-09-17 사용자 확인).
> 폰트는 Figma 텍스트 스타일에 찍힌 `Noto Sans KR` 대신 **Pretendard Variable**을
> 사용한다 — 플랫폼별 시스템 폰트(Apple SD Gothic Neo/맑은 고딕/Noto Sans KR 등)에
> 맞춰 자연스럽게 렌더링되도록 설계된 가변 폰트로, 사용자가 명시적으로 선택함.
>
> **추가 확인 (2026-09-18)**: Figma 파일에 실제 화면 목업 페이지(`02 · 전체 앱 — Light`,
> `03 · 전체 앱 — Dark`, `04 · 테마 비교 & 컬러 가이드`)가 있음이 뒤늦게 확인되어
> `01 · 디자인 시스템`(컴포넌트 라이브러리)에 더해 화면 단위 1순위 레퍼런스로 함께 사용한다
> (분석 결과는 세션 스크래치패드의 `figma-screen-analysis.md` 참고). 이에 따라 예외 범위를
> 다음과 같이 확장한다(2026-09-18 사용자 확인):
> - **Welcome 3단계 온보딩 재구축**: 현재 단일 화면을 Figma의 탭 체험 → 색 변화 정책 설명
>   → 공간 소개/로그인 3단계 캐러셀로 재구축한다.
>   Typography 체계 변경 금지는 화면 단위 heading 태그/크기 재조정에는 적용하지 않는다
>   (tailwind.config.js의 fontSize 토큰 자체 변경은 여전히 금지).
> - **경로 없는 Figma 화면 4개 신규 구현**: 기록 완료, 가입 완료, 도움말(`/help`),
>   공간 추가 완료. 각 화면의 소속 기존 화면(QuickRecord/SignUp/Settings/SpaceManage) 차례에
>   함께 만든다.
> - **동작/데이터 로직 변경 2건**: Home.tsx의 공간 색 바램(recency-fade)을 Figma 정책
>   (100/72/48/28% 4단계, 항목별 경과시간÷권장주기 기반, 아이콘/텍스트 항상 100% 유지,
>   구간 간 부드러운 보간)에 맞춰 정확히 구현. CareAction.tsx의 CTA를 QuickRecord로
>   이동시키는 대신 그 자리에서 바로 기록 처리하도록 변경하고 준비물 3종 행을 추가.
> - **다크모드는 이번 이니셔티브에서 제외**한다. Figma에 다크 팔레트가 모두 정의되어
>   있으나(`figma-screen-analysis.md` §(f) 참고), 지금은 라이트 모드만 구현한다.
>
> **추가 확인 (2026-09-24, Figma 전면 반영)**: Figma `02 · 전체 앱 — Light` 페이지를 화면 단위
> 1순위 레퍼런스로 삼아 앱 전체를 맞춘다(사용자 확인). 확정 사항:
> 1. 홈 공간 카드는 **v2**(`BB / Home Space Card`, 공간색 × 투명도 + 공간색 아이콘)를 쓴다.
>    recency-fade 정책(100/72/48/28%)은 v2 카드의 공간색 투명도에 적용한다.
> 2. 기록 완료 화면은 **29:520 `09 / 기록 완료`**(일러스트 버전)를 구현한다.
> 3. Figma `BB/*` 텍스트 스타일을 `tailwind.config.js`에 **새 fontSize 토큰(`bb-*`)으로 추가**하는 것을
>    허용한다(기존 fontSize 토큰의 변경/삭제는 여전히 금지).
> 4. 데이터 통합: 공간·항목·청소 기록의 단일 출처는 `src/data/cleaning.ts`이며, 어느 화면에서 남긴
>    기록이든 히스토리·항목 기록·홈 최신도에 반영된다. 날짜는 "지금" 기준으로 파생하고, 공간명
>    `침실`은 Figma 표기인 `방`으로 통일한다.
